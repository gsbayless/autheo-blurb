...
function initStaggerReveal(): void {
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          // When the first partner enters view, reveal all with stagger
          const grid = entry.target.closest('.partners-grid')
          if (grid) {
...
// --- Partner logo hover interactions ---
function initPartnerHovers(): void {
  const partners = document.querySelectorAll<HTMLElement>('[data-partner]')

  partners.forEach((el) => {
    el.addEventListener('mouseenter', () => {
      el.style.transform = 'scale(1.06)'
      el.style.borderColor = 'rgba(0, 229, 204, 0.2)'
